...
class VisualSlam:
    ...
    def estimate_current_camera_position(self, current_frame):
        ...
        for match in self.two_last_frames.matches_with_3d_information:
            self.feature_mapper[match.featureid2] = match.featureid1
            # Save all features matches in a dictionary for easy 
            # access later on.
            self.feature_history[match.featureid1] = match
        ...
        descriptors_from_current_frame = [feature.descriptor
                for feature 
                in current_frame.features]

        descriptors_in_map = [point.descriptor
                for point
                in self.map.points
                if np.linalg.norm(point.point) < 500]

        temp = self.bf.match(
                np.array(descriptors_from_current_frame),
                np.array(descriptors_in_map))

        matches_with_map = []
        for match in temp:
            image_feature = current_frame.features[match.queryIdx]
            map_feature = self.map.points[match.trainIdx]
        ...
        print(len(temp), len(matches_with_map))
        ...
        try: 
            ...
            if retval:
                ...
                for idx in inliers[:, 0].tolist():
                    match = matches_with_map[idx]

                    tp = self.mappointdict[match.featureid2]
                    observation = Observation(tp.point_id, 
                            camera.camera_id, 
                            match.imagecoord)
                    self.map.observations.append(observation)
    ...
    def match_current_and_previous_frame(self):
        ...
        ip = ImagePair(frame1, frame2, self.bf, self.camera_matrix)
        ip.match_features()
        essential_matches = ip.determine_essential_matrix(ip.filtered_matches)
        ...
        if len(self.list_of_frames) == 2:
            self.initialize_map(ip)

        self.two_last_frames = ip
