...
show = True;
def loadImage(name):
	return cv2.imread("./input/"+name);

def frame_viewer(filename):
	...
	# Skip first iteration
		if(i<1):
			frame1 = frame2;
			(kp1, des1) = extractSift(frame1);
			i = i+1;
			continue;

	# Manipulate frames
		# Extract sift features
		(kp2, des2) = extractSift(frame2);
		# Find matching points
		matches = matcher(des2, des1);
	...
	# Show frame
		if(show == True):
			...
			# Handle esc key to quit viewing
	...
	# Release capture device when loop ends
	cap.release();
...
def test():
	...
	# Hopefully draw 3D points
	visualize_3D(D3Points);

def main():
	global show;
	show == False;
	frame_viewer("../data/DJI_0199.MOV");
